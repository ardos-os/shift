...
//!
//! ```rust
//! use crate::animations::animation::{Animation, BasicAnimation, SequenceAnimation, ParallelAnimation, DelayAnimation};
//! use crate::animations::animation::easing::{linear, ease_in_out_cubic};
//! use crate::animations::{seq, all};
//!
...
pub struct FromToAnimation<F = fn(f32) -> f32>
where
	F: Fn(f32) -> f32,
{
	id: String,
...
impl<F> FromToAnimation<F>
where
	F: Fn(f32) -> f32,
{
	pub fn new(id: &str, duration: f32, from: f32, to: f32, easing: F) -> Self {
		Self {
			id: id.to_string(),
			duration,
			elapsed: 0.0,
...
impl<F> Animation for FromToAnimation<F>
where
	F: Fn(f32) -> f32,
{
	fn update(&mut self, delta: f32) -> Vec<(String, f32)> {
		if self.is_finished() && delta.is_sign_positive() {
			return vec![(self.id.clone(), self.to)];
		}
		let delta = if self.elapsed + delta < 0.0 {
			-self.elapsed
		} else {
		...
		let eased = if delta.is_sign_negative() {
			1.0 - (self.easing)(1.0 - progress)
		} else {
			(self.easing)(progress)
		};
		let value = self.from + (self.to - self.from) * eased;
		vec![(self.id.clone(), value)]
	}
...
#[macro_export]
macro_rules! all {
	() => {$crate::animations::animation::ParallelAnimation::new(vec![])};
    ( $( $x:expr ),* ) => {
        {
            let mut animations: Vec<Box<dyn $crate::animations::animation::Animation>> = Vec::new();
            $(
                animations.push(Box::new($x));
            )*
            $crate::animations::animation::ParallelAnimation::new(animations)
        }
    };
}
    ...
    ( $( $x:expr ),* ) => {
        {
            ...
            $crate::animations::animation::SequenceAnimation::new(animations)
        }
    };
}
